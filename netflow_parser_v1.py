...
class Flow(object):
    ...
    def covert_int_to_ipv4(self, addr):
...
class Flow1(Flow):
    ...
    def __str__(self):
        ret = "proto %d %s:%d > %s:%d %d bytes" %             (self.protocol, self.src_addr, self.src_port,              self.dst_addr, self.dst_port, self.octets)
        return ret
...
class NetFlowPacket:
    ...
    def __init__(self, data):
        ...
        if len(data) - self.hdr.LENGTH !=             (self.hdr.num_flows * flow_class.LENGTH):
...
addrs = socket.getaddrinfo(host, port, socket.AF_UNSPEC, socket.SOCK_DGRAM, 0, socket.AI_PASSIVE)
socks = []
